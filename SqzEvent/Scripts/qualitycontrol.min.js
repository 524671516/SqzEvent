...
myApp.onPageInit("qualitytestdetails", function (page) {
    ...
    PhotoBrowser("qualitytestdetails"); $("#edit-qt").on("click", function () { $("#edit-qt-form").ajaxSubmit(function (data) { if (data.result == "SUCCESS") { mainView.router.back(); myApp.addNotification({ title: "通知", message: "表单修改成功" }); setTimeout(function () { myApp.closeNotification(".notifications"); }, 2e3); } else { mainView.router.back(); myApp.addNotification({ title: "通知", message: "表单修改失败" }); setTimeout(function () { myApp.closeNotification(".notifications"); }, 2e3); } }); })
})
