var myApp = new Framework7({ modalTitle: '生产管理', pushState: true, }); var $$ = Dom7; var mainView = myApp.addView('.view-main', { dynamicNavbar: true, }); $$(document).on("ajaxStart", function (e) {
...
myApp.onPageInit('qccheckin', function (page) {
    ...
    if ($$("#FactoryId").val() != "") { $$.ajax({ url: "/QualityControl/QualityFactoryTestPartial", data: { fid: $$("#FactoryId").val() }, success: function (data) { $$("#factorytest-list").html(data); } }) }
    $$("#FactoryId").on("change", function () { if ($$("#FactoryId").val() != "") { $$.ajax({ url: "/QualityControl/QualityFactoryTestPartial", data: { fid: $$("#FactoryId").val() }, success: function (data) { $$("#factorytest-list").html(data); } }) } else { $$("#factorytest-list").html(""); } })
...
myApp.onPageInit("addbreakdown", function (page) {
    ...
    var $factoryselect = $("#factory-select")
    var $addbreakdownsubmit = $("#addbreakdown-submit"); var $addbreakdownform = $('#addbreakdown-form'); var pickerInline = myApp.picker({ input: '#BreakDownTimeTiny', formatValue: function (p, values, displayValues) { return values[0] + ':' + values[1]; }, toolbarCloseText: "关闭", value: ["10", "00"], cols: time_col }); $factoryselect.on("change", function () { $$("#QCEquipmentId").html(""); $$("#QCEquipmentId").append("<option>- 请选择 -</option>"); $$("#BreakDownTypeId").html(""); $$("#BreakDownTypeId").append("<option>- 请选择 -</option>"); $$("#QCEquipmentId").val(""); $$("#BreakDownTypeId").val(""); $$("#equipment-select .item-after").text("- 请选择 -"); $$("#breakdowntype-select .item-after").text("- 请选择 -"); if ($$("#FactoryId").val() != "") { $$.ajax({ url: "/QualityControl/RefreshEquipmentListAjax", data: { factoryId: $("#FactoryId").val() }, type: "post", success: function (data) { data = JSON.parse(data); if (data.result == "SUCCESS") { for (var i = 0; i < data.content.length; i++) { $$("#QCEquipmentId").append("<option value=\"" + data.content[i].Id + "\">" + data.content[i].Name + "</option>"); } } } }); } }); $equipmentselect.on("change", function () { $$("#BreakDownTypeId").html(""); $$("#BreakDownTypeId").append("<option>- 请选择 -</option>"); $$("#BreakDownTypeId").val(""); $$("#breakdowntype-select .item-after").text("- 请选择 -"); if ($$("#QCEquipmentId").val() != "") { $$.ajax({ url: "/QualityControl/RefreshBreakdownTypeListAjax", data: { equipmentId: $("#QCEquipmentId").val() }, type: "post", success: function (data) { data = JSON.parse(data); if (data.result == "SUCCESS") { for (var i = 0; i < data.content.length; i++) { $$("#BreakDownTypeId").append("<option value=\"" + data.content[i].Id + "\">" + data.content[i].Name + "</option>"); } } } }); } }); $addbreakdownform.validate({ debug: false, errorPlacement: function (error, element) { myApp.hideIndicator(); $addbreakdownsubmit.prop("disabled", false).removeClass("color-gray"); }, errorClass: "invalid-input", submitHandler: function (form) { CheckError("addbreakdown-submit", "addbreakdown-form") } }); $addbreakdownsubmit.on("click", function () { if (!$addbreakdownsubmit.prop("disabled")) { $addbreakdownsubmit.prop("disabled", true).addClass("color-gray"); setTimeout(function () { $addbreakdownform.submit(); }, 500); } }); uploadCheckinFile("addbreakdown-form", "addbreakdown-photos", "Photos", "addbreakdown-imgcount", 7); currentTextAreaLength("addbreakdown-form", "ReportContent", 200, "addbreakdown-currentlen");
