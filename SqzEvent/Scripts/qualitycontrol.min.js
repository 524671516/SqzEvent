...
myApp.onPageInit("qualitytestlist", function (page) { var calendarMultiple = myApp.calendar({ input: "#qualitytestlist-date", dateFormat: "yyyy-mm-dd", monthNames: monthNames, monthNamesShort: monthNamesShort, dayNames: dayNames, dayNamesShort: dayNamesShort, closeOnSelect: true }); $$("#qualitytestlist-date").on("change", function () { $$.ajax({ url: "/QualityControl/QualityTestListPartial", data: { date: $$("#qualitytestlist-date").val() }, success: function (data) { $$("#qualitytestlist-content").html(data); } }); }); $$("#qualitytestlist-content").on("deleted", ".swipeout", function (e) { $$.ajax({ url: "/QualityControl/DeleteQualityTest", data: { qtId: $$(e.target).attr("data-url") }, method: "post", success: function (data) { data = JSON.parse(data); if (data.result != "SUCCESS") { myApp.alert("删除失败"); } } }); }); }); myApp.onPageInit("addqualitytest", function (page) {
    ...
    $$("#ProductId").on("change", function () { $addqualitytestsubmit.prop("disabled", true).addClass("color-gray"); $$("#list-type-template").html(""); $$("#list-type-template").addClass("hidden"); if ($$("#ProductId").val() != "") { $$.ajax({ url: "/QualityControl/AddQualityTestPartial", data: { pid: $$("#ProductId").val(), }, success: function (data) { $$("#list-type-template").html(data); $$("#list-type-template").removeClass("hidden"); $addqualitytestsubmit.prop("disabled", false).removeClass("color-gray"); uploadCheckinFile("addqualitytest-form", "addqualitytest-photos", "Photos", "addqualitytest-imgcount", 5); currentTextAreaLength("addqualitytest", "Remark", 200, "qctest-currentlen"); $(".one_photo").each(function () { var pid = $(this).attr("Id"); var uid = $(this).parent().parent().find("ul").attr("Id"); var countid = $(this).parent().parent().parent().parent().find("abbr").attr("Id"); uploadCheckinFile("addqualitytest-form", uid, pid, countid, 12); }); $("#list-type-template .item-after").on("click", function () { var _self = $(this).parent().parent().parent().parent(); myApp.confirm("是否确认删除此检测项", "提示", function () { _self.remove(); }); return false; }) }, error: function (data) { myApp.alert("请求失败。") } }) } })
